...
function buildTree(files) {
	...
	function sumNode(node) {
		...
		if (kids.length > 0) {
			...
			node.fileCount =
				kids.reduce((s, k) => s + k.fileCount, 0) + node.fileCount;
		}
	}
	sumNode(root);
...
function DirectoryBar({ node, maxInLevel, onDrillDown }) {
	...
	const _pct = maxInLevel > 0 ? (node.total / maxInLevel) * 100 : 0;
	const dominantColor = node.writes >= node.reads ? C.pink : C.cyan;
	const _readPct = node.total > 0 ? (node.reads / node.total) * 100 : 0;
	const _writePct = node.total > 0 ? (node.writes / node.total) * 100 : 0;
	...
	const inner = (
		<>
			...
			<span
				style={{
					fontSize: 11,
					color: isDir ? C.yellow : C.textMuted,
					width: 14,
					textAlign: "center",
					flexShrink: 0,
				}}
			...
			<span
				style={{
					fontSize: 11,
					color: isDir ? C.textPrimary : C.textSecondary,
			...
			>
				<span style={{ color: C.cyan, width: 32, textAlign: "right" }}>
					{node.reads}r
				</span>
				<span style={{ color: C.pink, width: 32, textAlign: "right" }}>
					{node.writes}w
				</span>
				<span style={{ color: C.textMuted, width: 36, textAlign: "right" }}>
					{node.total}
				</span>
			</div>
	...
	return <div style={{ cursor: "default", ...sharedBarStyle }}>{inner}</div>;
}

function DrillDownView({ tree, onNavigate: _onNavigate }) {
				...
				>
					<button
						...
						style={{
							...
							background: "none",
							font: "inherit",
							color: path.length > 0 ? C.pink : C.textSecondary,
							fontFamily: "monospace",
							fontSize: 10,
				...
				{top.map((f) => {
					...
					return (
						<div
							key={f.relPath}
							role="img"
							aria-label={`${f.relPath}: ${f.reads} reads, ${f.writes} writes`}
...
function TopFilesTable({ files, maxTotal }) {
	...
	return (
		...
		>
			...
			{top.map((f, i) => {
				const short =
					f.relPath.length > 55 ? `...${f.relPath.slice(-52)}` : f.relPath;
				return (
					<div
						key={f.relPath}
						style={{
...
export default function Heatmap() {
	...
	const { files, prefix } = useMemo(
		() => stripCommonPrefix(rawFiles),
		[rawFiles],
	);
	const tree = useMemo(() => buildTree(files), [files]);
	const maxTotal =
		files.length > 0 ? Math.max(...files.map((f) => f.total)) : 0;
	...
	return (
		<div style={{ height: "100%", overflowY: "auto", padding: "20px 24px" }}>
			...
			{loading ? (
				...
				>
					No file access data yet. File attention patterns appear once agent
					sessions generate tool events.
				</div>
			) : (
				<>
					{/* Summary cards */}
