		...
		>
			...
			{expanded && (
				...
				>
					{entry.context && (
						<div
							style={{ fontSize: 10, color: C.textSecondary, marginBottom: 6 }}
						>
							<span style={{ color: C.textMuted }}>Context: </span>
							{entry.context}
						</div>
					)}
					{entry.outcome && (
	...
	return (
		...
		>
			...
			{!collapsed && (
				<div>
					{entries.map((entry) => (
						<DeadEndRow key={entry.approach} entry={entry} />
					))}
				</div>
			)}
		</div>
...
export default function DeadEndMap() {
	...
	const grouped = useMemo(() => {
		...
		for (const r of matching) {
			...
			for (const t of r.tools_involved ?? []) entry.tools.add(t);
		}
	...
	const totalRecurrences = grouped.reduce(
		(s, g) => s + g.entries.reduce((s2, e) => s2 + e.count, 0),
		0,
	);
	const maxRecurrence = grouped.reduce(
		(m, g) => Math.max(m, ...g.entries.map((e) => e.count)),
		0,
	);
			...
			{loading ? (
				...
				>
					...
					<div style={{ fontSize: 11 }}>
						Dead ends appear when Archivist captures failed approaches across
						sessions. They help you avoid repeating mistakes.
					</div>
				</div>
			) : (
				<>
					...
					<div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
						...
						<StatCard
							...
							value={maxRecurrence > 0 ? `${maxRecurrence}x` : "—"}
