    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{
          ...
          background: statusColor, boxShadow: `0 0 5px ${statusColor}88`,
        }} />

        <PluginBadge plugin={event.plugin} />

        <span style={{ fontSize: 11, color: C.textSecondary, fontWeight: 500, flex: 1, minWidth: 0,
      ...
      {expanded && hasMeta && (
        <div style={{
          marginTop: 8, marginLeft: 14, paddingLeft: 20,
          padding: "8px 10px", background: C.bg0,
...
function EmptyState() {
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column",
      alignItems: "center", justifyContent: "center", color: C.textMuted, gap: 12 }}>
      <div style={{ fontSize: 32 }}>⚡</div>
      <div style={{ fontSize: 14, color: C.textSecondary }}>Waiting for events</div>
      ...
      <a href="https://onlooker.dev/docs/setup" target="_blank" rel="noopener noreferrer"
...
export default function LiveFeed({ events, active }) {
  ...
  const filtered = displayEvents.filter((e) => {
    ...
    if (search && !`${e.label ?? ""} ${e.detail ?? ""}`.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });
  ...
  for (const [i, e] of filtered.entries()) {
    if (e.session && e.session !== lastSession) {
      rows.push({ type: "divider", sessionId: e.session, ts: e.ts, key: `div_${e.session}_${i}` });
      lastSession = e.session;
    }
    // Use index as tiebreaker so key is always unique even if ts/plugin/label are missing
    rows.push({ type: "event", event: e, key: `evt_${i}_${e.ts ?? ""}_${e.plugin ?? ""}` });
  }
      ...
      <div style={{ padding: "14px 20px 10px", borderBottom: `1px solid ${C.border}`,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: C.textPrimary }}>Live Feed</span>
          </div>
