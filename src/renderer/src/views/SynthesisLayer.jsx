...
import {
  useCostData, useInstructionHealth, useInstructionWatcher,
...
function buildTimeline(sessions, costRecords, health, instructionChanges) {
  ...
  for (const [day, daySessions] of Object.entries(sessionsByDay)) {
    const avgFriction = daySessions.filter((s) => s.friction != null).length > 0
      ? daySessions.filter((s) => s.friction != null)
          .reduce((s, x) => s + x.friction.score, 0) /
        daySessions.filter((s) => s.friction != null).length
      : null;

    const avgScore = daySessions.filter((s) => s.avgScore != null).length > 0
      ? daySessions.filter((s) => s.avgScore != null)
    ...
    if (avgFriction != null) {
      events.push({
        ...
        value: avgFriction,
        delta: null,
        color: avgFriction < 0.3 ? C.green : avgFriction < 0.6 ? C.yellow : C.red,
      });
    }
  ...
  if (health?.last_audit_at) {
    ...
    const totalIssues = (health.issue_count?.high ?? 0) +
      (health.issue_count?.medium ?? 0) + (health.issue_count?.low ?? 0);
    events.push({
      ts: health.last_audit_at,
      day,
      stream: "instructions",
      label: `Audit: ${Math.round((health.health_score ?? 0) * 100)}% health`,
      detail: totalIssues > 0 ? `${totalIssues} issue${totalIssues !== 1 ? "s" : ""}` : "clean",
...
function findCausalLinks(timeline) {
  ...
  const instructionDays = new Set(
    timeline.filter((e) => e.stream === "instructions" && e.label.startsWith("Edit"))
      .map((e) => e.day)
  );
  ...
  for (const day of instructionDays) {
    ...
    for (const e of timeline) {
      if ((e.day === nd1 || e.day === nd2) && (e.stream === "friction" || e.stream === "quality")) {
  ...
  return (
    ...
    >
      ...
      <span style={{
        ...
        fontFamily: "monospace", width: 16,
      }}>
        {streamDef?.icon ?? "·"}
      </span>
      <span style={{
        fontSize: 10, color: C.textSecondary, flex: 1,
    ...
    <div style={{ height: "100%", overflowY: "auto", padding: "20px 24px" }}>
      ...
      {timeline.length === 0 ? (
        <div style={{
          ...
          <div style={{ fontSize: 13, marginBottom: 8 }}>No synthesis data yet</div>
          <div style={{ fontSize: 11 }}>
            The synthesis layer connects instruction changes to behavior changes.
            It needs session data, cost records, and instruction file activity to build the timeline.
          </div>
        </div>
      ) : (
        <>
          ...
          <div style={{
            ...
            {causalLinks.length > 0 && (
              <span style={{
                display: "flex", alignItems: "center", gap: 4,
                fontSize: 9, color: C.textMuted, marginLeft: "auto",
              }}>
                <svg width="16" height="8">
                  <line x1="0" y1="4" x2="16" y2="4"
                    stroke={C.pink} strokeWidth="1.5" strokeDasharray="3,2" />
                </svg>
                {causalLinks.length} causal link{causalLinks.length !== 1 ? "s" : ""} detected
              </span>
            )}
          </div>
          ...
          {grouped.map(([day, dayEvents]) => {
            const hasLink = causalLinks.some((l) => l.from === day || l.to === day);
            ...
            return (
              <div key={day} style={{
                ...
                {/* Causal annotations */}
                {linksForDay.map((link, i) => (
                  <div key={i} style={{
                    fontSize: 9, color: C.pink, fontFamily: "monospace",
                    padding: "2px 8px", marginBottom: 4,
                    background: `${C.pink}0a`, borderRadius: 3,
                    borderLeft: `2px solid ${C.pink}`,
                  }}>
                    ← Possibly caused by instruction edit on {formatDay(link.from + "T12:00:00")}: {link.label}
                  </div>
                ))}
                ...
                <div style={{
                  ...
                  {dayEvents.map((e, i) => (
                    <TimelineEvent key={i} event={e} selected={selected} onSelect={setSelected} />
                  ))}
                </div>
              </div>
            );
