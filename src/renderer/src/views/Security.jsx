...
function Section({ title, children, style }) {
  return (
    <div style={{ background: C.bg2, borderRadius: 10, padding: "16px 18px",
      border: `1px solid ${C.border}`, marginBottom: 16, ...style }}>
...
function ThreatTimeline({ events }) {
  const { data, unit } = useMemo(() => {
    ...
    const labelFn  = useHours
      ? (k) => { const h = parseInt(k.slice(11, 13), 10);
                 return `${h % 12 || 12}${h >= 12 ? "p" : "a"}`; }
      : (k) => new Date(k + "T12:00:00").toLocaleDateString("en-US",
      ...
      >
        {data.map(({ key, blocks, allows, label }, i) => {
          ...
          return (
            <g key={key}>
              {allows > 0 && (
                <rect x={x} y={svgH - totalPx} width={barW} height={allowPx}
                  fill={C.green} opacity={0.55} rx={2} />
              )}
              {blocks > 0 && (
                <rect x={x} y={svgH - blockPx} width={barW} height={blockPx}
                  fill={C.red} opacity={0.85} rx={2} />
              )}
              {showLabel && (
                <text x={x + barW / 2} y={svgH + 13}
                  textAnchor="middle" fontSize={7} fill={C.textMuted} fontFamily="monospace">
                  {label}
                </text>
              )}
            </g>
...
function LegendDot({ color, label, opacity = 1 }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
      <div style={{ width: 8, height: 8, borderRadius: 1, background: color, opacity }} />
  ...
  if (patterns.length === 0) {
    return (
      <Section title="Matched Patterns" style={{ flex: 1 }}>
        <div style={{ color: C.textMuted, fontSize: 11 }}>No patterns recorded</div>
      </Section>
    );
  }
  ...
  return (
    <Section title="Matched Patterns" style={{ flex: 1 }}>
      {patterns.map(({ pattern, total, blocks }) => {
        ...
        const truncated = pattern.length > 38 ? pattern.slice(0, 35) + "…" : pattern;
        return (
...
function TriggerSources({ events }) {
  const sources = useMemo(() => {
    const counts = {};
    for (const e of events) {
      const raw = e.meta?.target ?? e.meta?.url ?? e.detail ?? "(unknown source)";
  ...
  if (sources.length === 0) {
    return (
      <Section title="Trigger Sources" style={{ flex: 1 }}>
        <div style={{ color: C.textMuted, fontSize: 11 }}>No sources recorded</div>
      </Section>
    );
  }
...
function ToolCoverage({ events }) {
  const tools = useMemo(() => {
    const counts = {};
    for (const e of events) {
      const tool = e.meta?.tool ?? e.tool_name ?? e.meta?.tool_name ?? "Unknown";
    ...
    <Section title="Tool Coverage">
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        {tools.map(({ tool, total, blocks }) => {
          ...
          return (
            <div key={tool} style={{ background: C.bg3, borderRadius: 8,
              padding: "10px 14px", border: `1px solid ${C.border}`,
              minWidth: 110, flex: 1 }}>
              <div style={{ fontSize: 11, color: C.textSecondary,
                fontFamily: "monospace", marginBottom: 4 }}>{tool}</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: C.orange,
                fontFamily: "'JetBrains Mono', monospace", lineHeight: 1 }}>{total}</div>
              <div style={{ fontSize: 9, color: blocks > 0 ? C.red : C.textMuted,
                marginTop: 3 }}>
...
function EventLog({ events }) {
  ...
  return (
    <Section title={`Event Log${events.length > 50 ? ` — showing 50 of ${events.length}` : ""}`}>
      {recent.map((e, i) => {
        ...
        return (
          <div
            ...
            onMouseEnter={(ev) => { if (!isOpen) ev.currentTarget.style.background = "#ffffff06"; }}
...
function MetaRow({ k, v }) {
  ...
  return (
    <div style={{ display: "flex", gap: 10, marginBottom: 5, alignItems: "flex-start" }}>
      ...
      <span style={{ fontSize: 10, color: C.textSecondary, fontFamily: "monospace",
        wordBreak: "break-all", whiteSpace: "pre-wrap" }}>{display}</span>
    </div>
  );
}
...
function EmptyState() {
  return (
    <div style={{ background: C.bg2, borderRadius: 10, padding: "48px 24px",
      border: `1px solid ${C.border}`, textAlign: "center" }}>
      <div style={{ fontSize: 32, marginBottom: 12, color: C.orange, opacity: 0.4 }}>⊘</div>
      <div style={{ fontSize: 13, color: C.textSecondary, marginBottom: 6 }}>
        No Warden events in this range
      </div>
      <div style={{ fontSize: 11, color: C.textMuted, maxWidth: 320, margin: "0 auto" }}>
        Warden monitors WebFetch and Read operations for indirect prompt injection attempts.
        Events appear here once Claude Code sessions are active.
      </div>
    </div>
  );
}
...
export default function Security({ sessions }) {
  ...
  const wardenEvents = useMemo(() =>
    allEvents.filter((e) => e.plugin === "warden"), [allEvents]);

  const blocks    = useMemo(() => wardenEvents.filter((e) => e.status === "block"), [wardenEvents]);
  const allows    = useMemo(() => wardenEvents.filter((e) => e.status !== "block"), [wardenEvents]);
  const blockRate = wardenEvents.length > 0
  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", padding: "20px 24px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: C.textPrimary }}>Security</span>
          <span style={{
