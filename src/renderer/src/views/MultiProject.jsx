...
export default function MultiProject({ sessions }) {
	...
	}, [sessions, costRecords, health?.cwd, cutoff]);
	...
	const sorted = useMemo(() => {
		...
		arr.sort((a, b) => {
			switch (sortKey) {
				case "sessions":
					return (a.sessions7d - b.sessions7d) * dir;
				case "cost":
					return (a.cost7d - b.cost7d) * dir;
				case "friction":
					return ((a.avgFriction ?? 0) - (b.avgFriction ?? 0)) * dir;
				case "warden":
					return (a.wardenEvents - b.wardenEvents) * dir;
				case "last":
					return (
						(new Date(a.lastActive || 0) - new Date(b.lastActive || 0)) * dir
					);
				default:
					return 0;
			}
		...
		<div style={{ height: "100%", overflowY: "auto", padding: "20px 24px" }}>
			...
			>
				...
				<span style={{ fontSize: 10, color: C.textMuted }}>
					{projects.length} project{projects.length !== 1 ? "s" : ""} · last 7
					days
				</span>
			</div>
			...
			{projects.length === 0 ? (
				...
				>
					...
					<div style={{ fontSize: 11 }}>
						Projects appear once agent sessions are recorded with working
						directory metadata.
					</div>
				</div>
			) : (
						...
						>
							...
							<span
								style={{
									...
									fontFamily: "monospace",
									color:
										p.cost7d > 5 ? C.red : p.cost7d > 1 ? C.yellow : C.green,
								}}
							>
								{fmtCost(p.cost7d)}
							</span>
