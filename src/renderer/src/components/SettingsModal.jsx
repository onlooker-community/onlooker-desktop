...
const C = {
  ...
  cyan: "#22d3ee", yellow: "#fbbf24", green: "#4ade80", red: "#f87171",
...
function SectionTitle({ children }) {
  return (
    <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.12em",
      color: C.textMuted, textTransform: "uppercase", fontFamily: "monospace",
      marginBottom: 12, marginTop: 22, paddingBottom: 6,
  ...
  return (
    <>
      <label style={{ display: "block", fontSize: 11, fontWeight: 600,
        color: C.textSecondary, marginBottom: 5, letterSpacing: "0.03em" }}>
        {label}
      </label>
      {hint && (
        <div style={{ fontSize: 10, color: C.textMuted, marginTop: 4, lineHeight: 1.5 }}>{hint}</div>
      )}
    </>
  );
}
  ...
  return (
    <input
      ...
      style={{
        ...
        fontSize: 12,
        fontFamily: monospace ? "'JetBrains Mono', monospace" : "'DM Sans', sans-serif",
...
function ActionBtn({ label, color, onClick, primary }) {
  return (
    <button onClick={onClick} style={{
      ...
      onMouseLeave={(e) => e.currentTarget.style.background = primary ? `${color}22` : "transparent"}
    >
      {label}
    </button>
  );
...
export default function SettingsModal({ settings, onSave, onClose }) {
  ...
  return (
    <div style={{
      position: "fixed", inset: 0, background: `${C.bg0}ee`,
      backdropFilter: "blur(8px)", zIndex: 200,
      display: "flex", alignItems: "center", justifyContent: "center",
      animation: "fadeIn 0.2s ease",
    }} onClick={onClose}>
      <div style={{
        background: C.bg1, border: `1px solid ${C.borderAccent}`,
        borderRadius: 14, width: 480, maxWidth: "90vw", maxHeight: "80vh",
        ...
        <div style={{ padding: "20px 24px 0", display: "flex",
          ...
          <div style={{ fontSize: 13, fontWeight: 700, color: C.textPrimary }}>Settings</div>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer",
            color: C.textMuted, fontSize: 18, padding: "2px 8px", transition: "color 0.15s" }}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "4px 24px 20px" }}>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 5 }}>
              {keyMasked ? (
                ...
                <div style={{ flex: 1, padding: "8px 10px", background: C.bg2,
                  border: `1px solid ${C.border}`, borderRadius: 6,
                  fontSize: 12, fontFamily: "monospace", color: C.textMuted }}>
                  {keyMasked}
                </div>
              ) : (
                <TextInput value={newKey} onChange={setNewKey} type="password"
                  placeholder="sk-ant-api03-…" monospace />
              )}
              {keyMasked
                ? <ActionBtn label="Replace"
                    color={C.yellow}
                    onClick={() => { setKeyMasked(null); setNewKey(""); }} />
                : <ActionBtn
                    label={keyStatus === "saved" ? "✓ Saved" : keyStatus === "error" ? "Error" : "Save"}
                    color={keyStatus === "saved" ? C.green : keyStatus === "error" ? C.red : C.pink}
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <select
              ...
              style={{ width: "100%", padding: "8px 10px", background: C.bg2,
                border: `1px solid ${C.border}`, borderRadius: 6,
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ marginTop: 5 }}>
              <TextInput value={String(form.maxTokens ?? 8096)}
                onChange={(v) => set("maxTokens", parseInt(v) || 8096)} monospace />
            </div>
          </div>
          ...
          <div style={{ marginBottom: 18 }}>
            <FieldLabel label="Log Directory"
            ...
            <div style={{ marginTop: 5 }}>
              <TextInput value={form.logDir ?? "~/.claude/onlooker"}
                onChange={(v) => set("logDir", v)} monospace />
            </div>
          </div>
          <div style={{ marginBottom: 18 }}>
            <FieldLabel label="Tribunal Quality Threshold"
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 8 }}>
              <input type="range" min={0} max={1} step={0.01}
                ...
                onChange={(e) => set("tribunalThreshold", parseFloat(e.target.value))}
                style={{ flex: 1, accentColor: C.pink }} />
              <span style={{ fontFamily: "monospace", fontSize: 12, color: C.textPrimary, width: 32, textAlign: "right" }}>
                {(form.tribunalThreshold ?? 0.75).toFixed(2)}
              </span>
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 18 }}>
            <Toggle value={form.sentinelStrict ?? true}
              onChange={(v) => set("sentinelStrict", v)}
              label="Sentinel strict mode (block destructive bash ops)" />
            <Toggle value={form.debugMode ?? false}
            ...
            <>
              ...
              {plugins.map((p) => (
                <div key={p.id} style={{ display: "flex", alignItems: "center",
                  padding: "7px 10px", background: C.bg2, borderRadius: 6,
                  border: `1px solid ${C.border}`, marginBottom: 6 }}>
                  <span style={{ fontSize: 11, color: C.textSecondary }}>{p.name ?? p.id}</span>
                  {p.version && (
                    <span style={{ marginLeft: "auto", fontSize: 9, color: C.textMuted, fontFamily: "monospace" }}>
                      v{p.version}
                    </span>
                  )}
                  <div style={{ width: 6, height: 6, borderRadius: "50%",
                    background: C.green, marginLeft: 8 }} />
                </div>
              ))}
            </>
          )}
        ...
        <div style={{ padding: "14px 24px", borderTop: `1px solid ${C.border}`,
          ...
          <ActionBtn label="Save Settings" color={C.pink} onClick={handleSave} primary />
        </div>
      </div>
    </div>
  );
