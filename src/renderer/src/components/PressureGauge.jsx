...
export default function PressureGauge({ pressure, active }) {
  ...
  const tooltip = open ? undefined : [
    `Context: ${pctLabel}`,
    hasPressure ? `${fmtTokens(pressure.inputTokens)} / ${fmtTokens(pressure.maxTokens)} tokens` : null,
  ].filter(Boolean).join("\n");
    ...
    <div style={{
      ...
      {!hasPressure ? (
        ...
        <>
          ...
          {/* Breakdown */}
          <div style={{
            display: "flex", gap: 16, fontSize: 10, color: "#94a3b8",
            paddingTop: 8, borderTop: "1px solid #1f2335",
            marginBottom: 8,
          }}>
          ...
          {pressure.lastUpdate && (
            <div style={{ fontSize: 9, color: "#475569", fontFamily: "monospace" }}>
              updated {new Date(pressure.lastUpdate).toLocaleTimeString("en-US", { hour12: false })}
            </div>
          )}
        </>
      )}
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 8 }}>
        <button
          ...
          style={{
            ...
            border: "1px solid #252a3d", background: "transparent",
