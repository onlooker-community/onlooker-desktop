...
const C = {
  ...
  cyan: "#22d3ee", yellow: "#fbbf24", purple: "#a78bfa", green: "#4ade80",
  textPrimary: "#e2e8f0", textSecondary: "#94a3b8", textMuted: "#475569",
};

const PLUGIN_COLORS = {
  sentinel: C.yellow, tribunal: C.pink, archivist: C.cyan, scribe: C.purple,
};

function PluginBadge({ plugin }) {
  const labels = { sentinel: "SEN", tribunal: "TRB", archivist: "ARC", scribe: "SCR" };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        fontSize: 11, color: isUser ? C.textMuted : C.pink,
      }}>
        {isUser ? "M" : "✦"}
      </div>
...
function StreamingBubble({ text }) {
  return (
    <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
      <div style={{
        width: 28, height: 28, borderRadius: "50%", flexShrink: 0,
        background: `${C.pink}22`, border: `1px solid ${C.pinkDim}`,
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 11, color: C.pink,
      }}>✦</div>
      <div style={{
        maxWidth: "74%", background: C.bg2,
        ...
        {text && (
          <span style={{
            ...
            background: C.pink, marginLeft: 1,
            verticalAlign: "text-bottom", animation: "pulse 0.8s infinite",
          }} />
        )}
      </div>
    </div>
...
export default function ChatPane({ messages, send, streaming, streamBuffer, debugMode }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 18px" }}>
        {messages.length === 0 && (
          <div style={{ textAlign: "center", marginTop: 60, color: C.textMuted }}>
            ...
            <div style={{ fontSize: 13 }}>Session started — Onlooker is watching.</div>
          </div>
        )}
        {messages.map((msg, i) => (
          <Message key={i} msg={msg} showDebug={debugMode} />
        ))}
      ...
      {/* Input area */}
      <div style={{ padding: "12px 18px", borderTop: `1px solid ${C.border}`, background: C.bg1 }}>
        <div
          style={{
          ...
          onFocusCapture={(e) => e.currentTarget.style.borderColor = C.borderAccent}
        ...
        >
          <textarea
            ...
            style={{
              ...
              fontFamily: "'DM Sans', sans-serif", maxHeight: 120,
        ...
        {/* Active plugin dots */}
        <div style={{ marginTop: 6, display: "flex", gap: 10, fontSize: 10, color: C.textMuted }}>
          {Object.entries(PLUGIN_COLORS).map(([plugin, color]) => (
            <span key={plugin} style={{ color }}>
              ● {plugin.charAt(0).toUpperCase() + plugin.slice(1)}
            </span>
          ))}
          <span style={{ marginLeft: "auto", fontFamily: "monospace" }}>⏎ send · ⇧⏎ newline</span>
        </div>
      </div>
    </div>
  );
