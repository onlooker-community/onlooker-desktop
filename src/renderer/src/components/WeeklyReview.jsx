...
function MiniBar({ value, color }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, flex: 1 }}>
      ...
      <span style={{ fontSize: 10, color: C.textMuted, fontFamily: "monospace", width: 28, textAlign: "right" }}>
        {(value * 100).toFixed(0)}%
      </span>
    </div>
  );
...
export default function WeeklyReview({ onClose }) {
    ...
    window.onlooker.logs.query({ from: weekAgo }).then((events) => {
      ...
      const derived = Object.entries(byDay)
        ...
        .map(([date, { events, scores }]) => ({
          ...
          score:  scores.length ? scores.reduce((a, b) => a + b) / scores.length : 0.8,
          turns:  events.length,
          flags:  events.filter((e) => e.status === "warn" || e.status === "fail").length,
        }));
  ...
  const avg         = displaySessions.reduce((a, s) => a + s.score, 0) / displaySessions.length;
  ...
  const displaySynthesis = synthesis ??
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRadius: 14, width: 500, maxWidth: "90vw",
        padding: 26, boxShadow: `0 0 60px ${C.pink}22, 0 24px 48px #0009`,
      }} onClick={(e) => e.stopPropagation()}>

        {/* Header */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22 }}>
          <div>
            <div style={{ fontSize: 13, color: C.pink, fontWeight: 700, letterSpacing: "0.1em",
              fontFamily: "monospace", textTransform: "uppercase" }}>Weekly Review</div>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer",
            color: C.textMuted, fontSize: 18, padding: "2px 8px", borderRadius: 4, transition: "color 0.15s" }}
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 20 }}>
              {[
                { label: "Avg Score",   value: avg.toFixed(2), color: scoreColor(avg) },
                { label: "Total Turns", value: totalTurns,     color: C.cyan },
                { label: "Flags",       value: totalFlags,     color: totalFlags > 3 ? C.red : C.yellow },
              ].map((stat) => (
                <div key={stat.label} style={{ background: C.bg2, borderRadius: 8,
                  padding: "10px 12px", border: `1px solid ${C.border}` }}>
                  <div style={{ fontSize: 20, fontWeight: 700, color: stat.color, fontFamily: "monospace" }}>
                    {stat.value}
                  </div>
            ...
            <div style={{ marginBottom: 18 }}>
              {displaySessions.map((session) => (
                <div key={session.date} style={{
                  ...
                  background: session.score === maxScore ? `${C.green}0a` : "transparent",
                  border: `1px solid ${session.score === maxScore ? C.green + "22" : "transparent"}`,
                }}>
                  <span style={{ width: 90, fontSize: 10, color: C.textMuted,
                    fontFamily: "monospace", flexShrink: 0 }}>{session.date}</span>
                  <MiniBar value={session.score} color={scoreColor(session.score)} />
                  <span style={{ fontSize: 10, color: C.textMuted, fontFamily: "monospace",
                    flexShrink: 0, width: 28 }}>{session.turns}t</span>
                  {session.flags > 0 && (
            ...
            {/* Synthesis blurb */}
            <div style={{ background: C.bg2, borderRadius: 8, padding: 14,
              border: `1px solid ${C.border}`, fontSize: 11, color: C.textSecondary,
            ...
            <div style={{ marginTop: 16, display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <ActionBtn label="Run Echo Regression" color={C.cyan}
                onClick={() => window.onlooker.plugins.run("echo", "run", [])} />
              <ActionBtn label="Counsel Synthesis" color={C.pink}
                onClick={() => window.onlooker.plugins.run("counsel", "weekly", [])} />
            </div>
          </>
        )}
      </div>
...
function ActionBtn({ label, color, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      color, cursor: "pointer", fontFamily: "monospace", letterSpacing: "0.05em",
