...
export function computeFriction(events) {
	...
	const oracleBad = oracleEvents.filter(
		(e) => e.status === "warn" || e.status === "block",
	).length;
	const oracle = {
		count: oracleBad,
		total: oracleEvents.length,
	...
	const guardEvents = events.filter(
		(e) => e.plugin === "warden" || e.plugin === "sentinel",
	);
	const guardBad = guardEvents.filter((e) => e.status === "block").length;
	const guard = {
		count: guardBad,
		total: guardEvents.length,
	...
	const tribunalBad = tribunalEvents.filter(
		(e) => e.status === "fail" || e.status === "warn",
	).length;
	const tribunal = {
		count: tribunalBad,
		total: tribunalEvents.length,
	...
	const score = active.reduce(
		(s, { signal, w }) => s + signal.ratio * (w / totalWeight),
		0,
	);
